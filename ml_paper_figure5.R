...
write.table(en, file="Z:/data/twas_mesa/en_cetp.txt", row.names=F,quote=F,sep="\t")
...
write.table(rf, file="Z:/data/twas_mesa/rf_cetp.txt", row.names=F,quote=F,sep="\t")
...
write.table(svr, file="Z:/data/twas_mesa/svr_cetp.txt", row.names=F,quote=F,sep="\t")
...
p1 <- ggplot(en, aes(x=cetp, y=pheno)) + geom_point(lwd=3)  +
  ...
  theme_classic(70) + ggtitle("Elastic Net") + geom_smooth(method="lm", color="red", lwd=3, se=TRUE) +
...
p2 <- ggplot(rf, aes(x=cetp, y=pheno)) + geom_point(lwd=3)  +
  xlab("predicted expression") + ylab("HDL (rank normalized)") +
  theme_classic(70) + ggtitle("Random Forest") + geom_smooth(method="lm", color="red", lwd=3, se=TRUE) +
  geom_density_2d(lwd=2.5)#(save at width=850, height=600) + xlim(0,1) + ylim(0,1)
#+ geom_abline(intercept=0, slope=1, color="blue", lwd=2) + geom_smooth(method="lm", color="red")
...
p3 <- ggplot(svr, aes(x=cetp, y=pheno)) + geom_point(lwd=3)  +
  xlab("predicted expression") + ylab("HDL (rank normalized)") +
  theme_classic(70) + ggtitle("Support Vector") + geom_smooth(method="lm", color="red", lwd=3, se=TRUE) +
  geom_density_2d(lwd=2.5)#(save at width=850, height=600) + xlim(0,1) + ylim(0,1)
#+ geom_abline(intercept=0, slope=1, color="blue", lwd=2) + geom_smooth(method="lm", color="red")
...
ggplot(data=df, aes(x=cetp, y=pheno)) + geom_point(lwd=3) + xlab("Predicted gene expression") +
  ylab("HDL (rank normalized)") + geom_smooth(method="lm", color="red", lwd=3, se=TRUE) +
  geom_density_2d(lwd=2.5) + theme_classic(50) + scale_x_continuous(breaks=seq(-2.0, 2.0, 1)) +
  facet_wrap(~method)
#width=2300 height=1700
